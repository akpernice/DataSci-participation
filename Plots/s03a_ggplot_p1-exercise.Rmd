...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) + geom_point(alpha= 0.1, color = 'purple') + scale_x_log10("Gdp Per Capita", labels = scales::dollar_format()) + theme_minimal() + ylab("Life Expectancy")
...
```{r}
p = ggplot(mauna, aes(month, conc)) + geom_line(color = "forest green") + ylab('Concentration')
...
```{r, fig.width = 5, fig.height = 2}
p <- ggplot(mauna, aes(x= month, y = conc) + geom_line()

p + geom_point(colour = 'forest green')
```
...
```{r, fig.width = 5, fig.height = 2}
ggplot(gapminder, aes(gdpPercap,lifeExp)) +  geom_line()
